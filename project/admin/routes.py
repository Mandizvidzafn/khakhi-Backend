...
def add_product():
    ...
    if request.method == "POST":
        ...
        if image and allowed_file(image.filename):
            ...
            unique_img_name = str(uuid4()) + "-" + image_name
            image_path = os.path.join(
                "project", "static", "img", "products", unique_img_name
            )
        ...
        if product:
            ...
            category_types_obj = [
                Category_type.query.get(int(cat_type_id))
                for cat_type_id in category_types
            ]
...
# delete products
@admin_routes.route("/admin/products/delete/<int:id>", methods=["GET", "POST"])
...
def view_product(id):
    ...
    return render_template(
        "admin/view_product.html", product=product, image_path=image_path
    )
...
def category():
        ...
        if "addCategorytype" in request.form:
            ...
            if not catType:
                if addcat_type != "":
                    ...
                    flash("PLease enter the category type", "error")
            else:
                flash(f"Category Type {addcat_type.upper()} exists", "danger")
        ...
        # Add Category
            ...
            if category and category.cat_type == cat_type:
                ...
                if cat_name == "" or cat_type == None:
                    ...
                    add_category = Category(name=cat_name, cat_type=cat_type)
    ...
    return render_template(
        "admin/category.html", category_type=category_types, categories=categories
    )
